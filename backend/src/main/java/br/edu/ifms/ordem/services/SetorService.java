...
public class SetorService {
	...
	//a assinatura do setor define o que ele está devolvendo, o carimbo
	@Transactional(readOnly = true) //importar do spring
	public List<SetorDTO> findAll(){	
		List<Setor> list = repository.findAll();
		return list.stream().map(s -> new SetorDTO(s)).collect(Collectors.toList());
	}
	...
	public SetorDTO findById(Long id) {
		...
		Setor entity = obj.orElseThrow(() -> new ResourceNotFoundException("O setor consultado não foi localizado"));
	...
	public SetorDTO update(Long id, SetorDTO dto) {
    	try {
    		...
    		throw new ResourceNotFoundException("O setor com o ID = "+id+" não foi localizado");    		
    	}
	}
